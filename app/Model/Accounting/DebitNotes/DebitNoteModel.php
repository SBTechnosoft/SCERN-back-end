<?php
namespace ERP\Model\Accounting\DebitNotes;
...
class DebitNoteModel extends Model
{
	...
	protected $child_table = 'debit_note_dtl';
	...
	function insertData($insertArray) {
		$now = Carbon::now();
		$dtl_array = $insertArray['debit_array'];
		$insertArray = array_except($insertArray, ['debit_array']);
		...
		try {
			...
			$debitId = $raw[0]->insertId;
			
			$dtlValues = call_user_func_array('array_merge', array_map(function($br) use($debitId, $now) {
				$ar = array();
				$ar[0] = $debitId;
				$ar[2] = $br['vendor_name'];
				$ar[3] = $br['ledger_id'];
			...
			$dtlKey = 'debit_id, vendor_name, ledger_id, amount, created_at';
