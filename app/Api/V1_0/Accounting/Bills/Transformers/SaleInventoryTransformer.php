...
class SaleInventoryTransformer
{
	...
	public function trimInventory($inventoryJson, $saleId)
	{
		...
		foreach ($inventory as $inv_record) {
			...
			$single['cess_percentage'] = is_numeric($inv_record['cessPercentage']) ? floatval(trim($inv_record['cessPercentage'])) : 0;
			$single['cess_flat'] = is_numeric($inv_record['cessFlat']) ? floatval(trim($inv_record['cessFlat'])) : 0;
			$single['cess_amount'] = is_numeric($inv_record['cessAmount']) ? floatval(trim($inv_record['cessAmount'])) : 0;
