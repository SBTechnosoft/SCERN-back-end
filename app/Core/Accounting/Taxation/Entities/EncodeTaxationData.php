...
class EncodeTaxationData extends ProductService
{
	...
	*/
	 ...
	 * returns the converted arraydata
	*/
	public function getGSTR3BData($inwardSupplies, $outwardSupplies, $companyId, $headers)
	{
		$exception = new ExceptionMessage();
		$exceptionArray = $exception->messageArrays();
		$inwardArray = array();
		...
		$interStateArray = array();
		$companyJson = $companyService->getCompanyData($companyId);
		if (strcmp($companyJson,$exceptionArray['404'])==0) 
		{
			return $companyJson;
		}
		$companyArray = json_decode($companyJson,true);
		$companyState = $companyArray['state']['stateAbb'];
		// For Sample
		// $interStateArray[$companyState] = array(
		// 	'place_of_supply' => $companyArray['state']['stateName'],
		...
		// 	'uin_tax' => 0,
		// );
		// GSTR Reports table wise default array generation
		...
		$GSTR3B = array(
			'outward' => array(
				...
				array(
					'particulars' => '(b) Outward Taxable  supplies  (zero rated )',
					'taxable_value' => 0,
					'integrated_tax' => 0,
					'central_tax' => 0,
					'state_tax' => 0,
				...
				array(
					'particulars' => '(c) Other Outward Taxable  supplies (Nil rated, exempted)',
					'taxable_value' => 0,
					'integrated_tax' => 0,
					'central_tax' => 0,
					'state_tax' => 0,
				...
				array(
					'particulars' => '(d) Inward supplies (liable to reverse charge)',
					'taxable_value' => 0,
					'integrated_tax' => 0,
					'central_tax' => 0,
					'state_tax' => 0,
				...
				array(
					'particulars' => '(e) Non-GST Outward supplies',
					'taxable_value' => 0,
					'integrated_tax' => 0,
					'central_tax' => 0,
					'state_tax' => 0,
			...
			'itc' => array(
				...
				array(
					'title' => '(D) Ineligible ITC',
					'details' => array(
						array(
							'particulars' => '(1)   As per section 17(5) of CGST//SGST Act',
							'integrated_tax' => 0,
							'central_tax' => 0,
							'state_tax' => 0,
							'cess' => 0
			...
			'exempt_inward' => array(
				array(
					'particulars' => 'From a supplier under composition scheme, Exempt  and Nil rated supply',
					'inter_state_supplies' => 0,
					'intra_state_supplies' => 0
				),
				array(
					'particulars' => 'Non GST supply',
					'inter_state_supplies' => 0,
					'intra_state_supplies' => 0
				)
			),
			'inter_state_supplies' => array()
		);
		$inwardArray = json_decode($inwardSupplies, true);
		$outwardArray = json_decode($outwardSupplies, true);
		if (!is_array($inwardArray)) 
		{
			$inwardArray = array();
		}
		if (!is_array($outwardArray)) 
		{
			$outwardArray = array();
		}

		// Outward Supply Report
		$totalOutwards = count($outwardArray);
		for ($outwardCount=0; $outwardCount < $totalOutwards; $outwardCount++) 
		{ 
			$inventoryArray = array();
			
			$intraStateFlag = $outwardArray[$outwardCount]['place_of_supply'] == $companyState;

			$productArray = json_decode($outwardArray[$outwardCount]['product_array'],true);
			$inventoryArray = $productArray['inventory'];

			if (!is_array($inventoryArray)) 
			{
				$inventoryArray = array();
			}
			
			$totalInv = count($inventoryArray);
			$outwardTaxable = array(
				'taxable' => 0,
				'igst' => 0,
				'cgst' => 0,
				'sgst' => 0,
			...
			for ($invCount=0; $invCount < $totalInv; $invCount++) 
			{
				// Add full Total in taxable first
				$amount = (float)$inventoryArray[$invCount]['amount'];
				// deduct tax amount to get real taxable value
				if (isset($inventoryArray[$invCount]['cessAmount'])) 
				{
					$amount -= (float)$inventoryArray[$invCount]['cessAmount'];
					$outwardTaxable['cess'] += (float)$inventoryArray[$invCount]['cessAmount'];
				}
				if ($intraStateFlag) 
				{
					if (isset($inventoryArray[$invCount]['cgstAmount']) && isset($inventoryArray[$invCount]['sgstAmount'])) 
					{
						
						if ((float)$inventoryArray[$invCount]['cgstAmount'] != 0 || (float)$inventoryArray[$invCount]['sgstAmount'] != 0) 
						{
							$outwardTaxable['cgst'] += (float)$inventoryArray[$invCount]['cgstAmount'];
							$amount -= (float)$inventoryArray[$invCount]['cgstAmount'];
							$outwardTaxable['sgst'] += (float)$inventoryArray[$invCount]['sgstAmount'];
							$amount -= (float)$inventoryArray[$invCount]['sgstAmount'];
							$outwardTaxable['taxable'] += $amount;
						}
						else 
						{
							$zeroRated['taxable'] += $amount;
							$zeroRated['cess'] += $outwardTaxable['cess'];
						}
						
					}
					else 
					{
						$nillRated['taxable'] += $amount;
						$nillRated['cess'] += $outwardTaxable['cess'];
					}
				}
				else 
				{
					if (isset($inventoryArray[$invCount]['igstAmount'])) 
					{
						
						if ($inventoryArray[$invCount]['igstAmount'] != 0) 
						{
							$outwardTaxable['igst'] += (float)$inventoryArray[$invCount]['igstAmount'];
							$amount -= (float)$inventoryArray[$invCount]['igstAmount'];
							$outwardTaxable['taxable'] += $amount;
						}
						else 
						{
			...
			// 3.1 (a)
			$GSTR3B['outward'][0]['taxable_value'] += $outwardTaxable['taxable'];
			$GSTR3B['outward'][0]['integrated_tax'] += $outwardTaxable['igst'];
			$GSTR3B['outward'][0]['central_tax'] += $outwardTaxable['cgst'];
			$GSTR3B['outward'][0]['state_tax'] += $outwardTaxable['sgst'];
			$GSTR3B['outward'][0]['cess'] += $outwardTaxable['cess'];
			// 3.2
			if (!$intraStateFlag) 
			{
				if (!isset($interStateArray[$outwardArray[$outwardCount]['place_of_supply']])) 
				{
					$stateStatus = $stateService->getStateData($outwardArray[$outwardCount]['place_of_supply']);
					$stateDecodedJson = json_decode($stateStatus,true);

					$interStateArray[$outwardArray[$outwardCount]['place_of_supply']] = array(
						'place_of_supply' => $stateDecodedJson['stateName'],
						'unregistered_taxable' => 0,
						'unregistered_tax' => 0,
						'composition_taxable' => 0,
						'composition_tax' => 0,
				...
				if ($outwardArray[$outwardCount]['gstin'] != '') 
				{
					$interStateArray[$outwardArray[$outwardCount]['place_of_supply']]['uin_taxable'] += $outwardTaxable['taxable'];
					$interStateArray[$outwardArray[$outwardCount]['place_of_supply']]['uin_tax'] += $outwardTaxable['igst'];
				}
				else
				{
					$interStateArray[$outwardArray[$outwardCount]['place_of_supply']]['unregistered_taxable'] += $outwardTaxable['taxable'];
					$interStateArray[$outwardArray[$outwardCount]['place_of_supply']]['unregistered_tax'] += $outwardTaxable['igst'];
				}
			}
			
			// 3.1 (b)
			$GSTR3B['outward'][1]['taxable_value'] += $zeroRated['taxable'];
			$GSTR3B['outward'][1]['integrated_tax'] += $zeroRated['igst'];
			$GSTR3B['outward'][1]['central_tax'] += $zeroRated['cgst'];
			$GSTR3B['outward'][1]['state_tax'] += $zeroRated['sgst'];
			$GSTR3B['outward'][1]['cess'] += $zeroRated['cess'];
			// 3.1 (e)
			$GSTR3B['outward'][4]['taxable_value'] += $nillRated['taxable'];
			$GSTR3B['outward'][4]['integrated_tax'] += $nillRated['igst'];
			$GSTR3B['outward'][4]['central_tax'] += $nillRated['cgst'];
			$GSTR3B['outward'][4]['state_tax'] += $nillRated['sgst'];
			$GSTR3B['outward'][4]['cess'] += $nillRated['cess'];
		}
		$GSTR3B['inter_state_supplies'] = $interStateArray;

		// Inward Supply Report

		$totalInwards = count($inwardArray);
		for ($inwardCount=0; $inwardCount < $totalInwards; $inwardCount++) 
		{
			$inventoryArray = array();
			$intraStateFlag = $inwardArray[$inwardCount]['supplier_state'] == $companyState;

			$productArray = json_decode($inwardArray[$inwardCount]['product_array'],true);
			$inventoryArray = $productArray['inventory'];
			...
			$exemptKey = $intraStateFlag ? 'intra_state_supplies' : 'inter_state_supplies';
			for ($invCount=0; $invCount < $totalInv; $invCount++) 
			{ 
				$amount = (float)$inventoryArray[$invCount]['amount'];
				if (isset($inventoryArray[$invCount]['cessAmount'])) 
				{
					$GSTR3B['itc'][0]['details'][4]['cess'] += (float)$inventoryArray[$invCount]['cessAmount'];
					$amount -= (float)$inventoryArray[$invCount]['cessAmount'];
				}

				if (isset($inventoryArray[$invCount]['cgstAmount']) || isset($inventoryArray[$invCount]['sgstAmount']) || isset($inventoryArray[$invCount]['igstAmount'])) 
				{
					...
					{
						$GSTR3B['exempt_inward'][0][$exemptKey] += $amount;
					}
				}
				else
				{
					$GSTR3B['exempt_inward'][1][$exemptKey] += $amount;
		...
		$GSTR3B['itc'][2]['details'][0]['cess'] = array_sum($cessArray0) - array_sum($cessArray1);
		return json_encode($GSTR3B);
	}
	 ...
	 * returns the converted arraydata
	*/
	public function getGstr2Data($taxationData, $companyId)
	{
		$exception = new ExceptionMessage();
		$exceptionArray = $exception->messageArrays();
		$inwardArray = array();
		$inwardArray = json_decode($taxationData, true);
		...
		$stateArray[$companyState] = $companyArray['state'];
		// Default GSTR2 Array
		$GSTR2 = array(
			'b2b' => array(),
			'b2c' => array(),
		...
		// Default Array Structures
		$b2cSampleArray = array(
			'supplier' => '',
			'invoice_no' => '',
			'invoice_date' => '',
			'invoice_value' => 0,
		...
		for ($inwardCount=0; $inwardCount < $totalInwards; $inwardCount++) 
		{
			...
			$supplyState = $inwardArray[$inwardCount]['supplier_state'];
			$intraStateFlag = $supplyState == $companyState;

			if (!isset($stateArray[$supplyState])) 
			{
				$stateResponse = $stateService->getStateData($supplyState);
				$stateArray[$supplyState] = json_decode($stateResponse, true);
			}
			
			$productArray = json_decode($inwardArray[$inwardCount]['product_array'],true);
			$inventoryArray = $productArray['inventory'];
			$purchaseId = $inwardArray[$inwardCount]['purchase_id'];
			...
			if ($inwardArray[$inwardCount]['gstin'] == '' && $inwardArray[$inwardCount]['tax'] == 0 && count($inventoryArray)) 
			{
				$GSTR2['b2c'][$purchaseId] = array(
					'supplier' => $inwardArray[$inwardCount]['ledger_name'],
					'invoice_no' => $inwardArray[$inwardCount]['bill_number'],
					'invoice_date' => $inwardArray[$inwardCount]['entry_date'],
					'invoice_value' => $inwardArray[$inwardCount]['total'],
					'place_of_supply' => $stateArray[$supplyState]['stateName'],
					'cess' => 0
				);
				continue;
			}
			...
			for ($invCount=0; $invCount < $totalInv; $invCount++) 
			{ 
				// $taxationModel->productType()

				$productId = $inventoryArray[$invCount]['productId'];
				if (!isset($productArray[$productId])) 
				{
					$productStatus = $taxationModel->productType($productId);
					$productArray[$productId] = json_decode($productStatus, true);
				}
				$serviceFlag = $productArray[$productId]['product_type']=='service';

				$amount = (float)$inventoryArray[$invCount]['amount'];
				$integrated_tax = (float)$inventoryArray[$invCount]['igstAmount'];
				$amount -= (float)$inventoryArray[$invCount]['igstAmount'];
				$state_tax = (float)$inventoryArray[$invCount]['sgstAmount'];
				$amount -= (float)$inventoryArray[$invCount]['sgstAmount'];
				$central_tax = (float)$inventoryArray[$invCount]['cgstAmount'];
				$amount -= (float)$inventoryArray[$invCount]['cgstAmount'];
				$cess = 0;
				if (isset($inventoryArray[$invCount]['cessAmount'])) 
				{
					$cess = (float)$inventoryArray[$invCount]['cessAmount'];
					$amount -= (float)$inventoryArray[$invCount]['cessAmount'];
				}
				...
				{
					$affectiveArray = 'imps';
					if (!isset($GSTR2['imps'][$purchaseId])) 
					{
						$GSTR2['imps'][$purchaseId] = array(
							'invoice_no' => $inwardArray[$inwardCount]['bill_number'],
							'invoice_date' => $inwardArray[$inwardCount]['entry_date'],
							'invoice_value' => $inwardArray[$inwardCount]['total'],
							'place_of_supply' => $stateArray[$supplyState]['stateName'],
				...
				elseif ($inwardArray[$inwardCount]['gstin'] == '') 
				{
					$affectiveArray = 'b2bur';
					if (!isset($GSTR2['b2bur'][$purchaseId])) 
					{
						$GSTR2['b2bur'][$purchaseId] = array(
							'supplier' => $inwardArray[$inwardCount]['ledger_name'],
							'invoice_no' => $inwardArray[$inwardCount]['bill_number'],
							'invoice_date' => $inwardArray[$inwardCount]['entry_date'],
							'invoice_value' => $inwardArray[$inwardCount]['total'],
				...
				elseif (!isset($GSTR2[$affectiveArray][$purchaseId])) 
				{
					$GSTR2[$affectiveArray][$purchaseId] = array(
						'gstin' => $inwardArray[$inwardCount]['gstin'],
						'invoice_no' => $inwardArray[$inwardCount]['bill_number'],
						'invoice_date' => $inwardArray[$inwardCount]['entry_date'],
						'invoice_value' => $inwardArray[$inwardCount]['total'],
						'place_of_supply' => $stateArray[$supplyState]['stateName'],
				...
				$GSTR2[$affectiveArray][$purchaseId]['taxable_value'] += $amount;
				$GSTR2[$affectiveArray][$purchaseId]['integrated_tax'] += $integrated_tax;
				$GSTR2[$affectiveArray][$purchaseId]['state_tax'] += $state_tax;
				$GSTR2[$affectiveArray][$purchaseId]['central_tax'] += $central_tax;
				$GSTR2[$affectiveArray][$purchaseId]['cess'] += $cess;
			}
		}
		$GSTR2['b2b'] = array_values($GSTR2['b2b']);
		$GSTR2['b2c'] = array_values($GSTR2['b2c']);
		$GSTR2['b2bur'] = array_values($GSTR2['b2bur']);
		$GSTR2['imps'] = array_values($GSTR2['imps']);
		
		return json_encode($GSTR2);
	}
}